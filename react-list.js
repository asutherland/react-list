  ...
  var QuantizedHeightList = (function (_List2) {
    ...
    _createClass(QuantizedHeightList, [{
      ...
      value: function updateFrame(event) {
        ...
        var maxOffset = Math.max(0, totalHeight - 1);

        var firstOffset = Math.min(maxOffset, Math.floor(top / unitSize));
        var firstVisibleOffset = Math.min(maxOffset, Math.floor(scroll / unitSize));
        var lastVisibleOffset = Math.min(maxOffset, Math.ceil((scroll + viewportHeight) / unitSize));
        var lastOffset = Math.min(maxOffset, Math.ceil((top + viewportHeight + threshold) / unitSize));
